...
def test_chat_interface(endpoint: str, message: str) -> bool:
    """Test the chat interface (Argus natural language)"""
    print(f"\n{'='*60}")
    print("5. Testing Chat Interface")
    print(f"{'='*60}")
...
def main():
    ...
    if not endpoint:
        if len(sys.argv) > 1:
            ...
            print("Usage: python test_root_agent_integration.py <endpoint_url>")
    ...
    print(f"\n{'='*60}")
    print(f"Testing Argus Root Orchestrator Agent")
    ...
    results.append(("Chat Interface", test_chat_interface(endpoint, "Analyze IP 8.8.8.8")))
